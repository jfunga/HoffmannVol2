...
% (Seguridad) Carga TikZ solo si no está cargado; este bloque usa un \tikz\node
\makeatletter
\@ifpackageloaded{tikz}{}{\usepackage{tikz}}
\makeatother
% Nota: la librería 'decorations.pathreplacing' ya se carga arriba para llaves

% --- Contador de casos: reinicia en cada capítulo ---
%   Uso: Cada \begin{CasoL}...\end{CasoL} incrementa el contador.
\newcounter{caso}[chapter]
\renewcommand{\thecaso}{\arabic{caso}} % 1,2,3... (sin prefijo de capítulo)

% (Opcional) Anchos paramétricos para la variante 2 columnas
%   Ajusta aquí si alguna vez quieres más/menos ancho sin cambiar los capítulos.
\newcommand{\casoLeftW}{0.58\linewidth}   % texto (izquierda)
\newcommand{\casoRightW}{0.38\linewidth}  % matemática (derecha)

% ===== Caso (variante ancho completo) =====
%   Uso:
%     \begin{CasoL}[Título opcional]
%       ... líneas \casolinea / \casolineaW ...
%     \end{CasoL}
%
%   Render:
%     Encabezado amarillo "Caso N — Título", luego el contenido.
...
% Alias para compatibilidad: 'Caso' == 'CasoL'
...
%   Comportamiento:
%     - Muestra el texto arriba (alineado a la izquierda)
%     - Debajo, centra la matemática en su propia línea
\NewDocumentCommand{\casolineaW}{s m m}{%
  \noindent\hspace*{1.5em}%
  \begin{minipage}{0.92\linewidth}% bloque ANCHO COMPLETO
    #2\par\vspace{0.4ex}%
    \noindent
    \IfBooleanTF{#1}{%
      % versión con *: NO envolver en \[ \] (ya traes display)
      #3%
    }{%
      % versión sin *: envolver en display math y centrar en su línea
      \[
        \displaystyle #3
      \]%
    }%
...
%   Comportamiento:
%     - Columna izq. (texto) ancho \casoLeftW
%     - Columna der. (mates)  ancho \casoRightW, centrada en display si no usas *
\NewDocumentCommand{\casolinea}{s m m}{%
  \noindent
  \begin{minipage}[t]{\casoLeftW}
    #2%
  \end{minipage}\hfill
  \begin{minipage}[t]{\casoRightW}
    \IfBooleanTF{#1}{%
      % versión con *: NO envolver (traes display propio)
      #3%
    }{%
      % versión sin *: display centrado en la columna
      \[
        \displaystyle #3
      \]%
    }%
